...
sub _bits_for_number {
    ...
    #return _d2b(  );
    return    int( length $n * 3.32192809488736 + 1 )
}
...
sub precision {
    my ( $self, $lat, $lon ) = @_;
    my $lab = int( length $lat * 3.32192809488736 + 1 ) + 8;
    my $lob = int( length $lon * 3.32192809488736 + 1 ) +  + 9;
...
#    my $prec = shift || $self->precision( @pos );
	...
	my $lab = int( length $lat * 3.32192809488736 + 1 ) + 8;
	my $lob = int( length $lon * 3.32192809488736 + 1 ) + 9;
	$prec = int( ( ( $lab > $lob ? $lab : $lob ) + 1 ) / 2.5 );
    }
...
#            my $mid = _mid( $int, $flip );
	    my $mid =  ( $int->[$flip][0] + $int->[$flip][1] ) / 2;
