# $Id: RSS.pm,v 1.34 2004/10/09 14:10:44 bhammersley Exp $
package XML::RSS;
...
sub handle_char {
	...
	my ($self,$cdata) = (@_);
#	warn "Check" . Dump($self->current_element);
	...
	# item element
		...
		if ($self->within_element($self->generate_ns_name("where","http://www.georss.org/georss")))
		{
		    $self->{'items'}->[$self->{num_items}-1]->{"where"} ->handle_char($self,$cdata);
		}
		elsif (		# If it's in the default RSS 1.0 namespace
			(!$ns && !$self->{rss_namespace}) ||
			($ns eq $self->{rss_namespace})
		) {
...
sub handle_start {
	...
	if ($ns eq 'http://www.georss.org/georss')
	{
	    if ($el eq "where")
	    {
		$self->{'items'}->[$self->{num_items}-1]->{ $el } = XML::RSS::GeoRSS->new();
	    }


	}
	elsif ($self->within_element($self->generate_ns_name("where","http://www.georss.org/georss")))
	{
	    $self->{'items'}->[$self->{num_items}-1]->{ "where" }-> handle_start (
	        $self,
		$el,
		\%attribs);
...
sub handle_end {
    ...
    if ($self->within_element($self->generate_ns_name("where","http://www.georss.org/georss")))
    {
	$self->{'items'}->[$self->{num_items}-1]->{ "where" }-> handle_end (
	    $self,
	    $el);
...
latest sources in CVS, as well as all of the previous releases.
	...
	http://sourceforge.net/projects/perl-rss/
