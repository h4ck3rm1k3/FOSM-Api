...
#for(keys %GeoHash::) { # All the symbols in Foo's symbol table
#  print "$_\n" if defined &{$_}; # check if symbol is method
#}

#for(keys %::) { # All the symbols in Foo's symbol table
#  print "test:$_\n"; # check if symbol is method
#}

#everzthing : warn Dump(\%::);

use strict 'refs';


#use Inline CPP CONFIG => (INC => '-I/usr/include/stlport/', BUILD_NOISY => 1, BUILD_TIMERS => 1 , REPORTBUG =>1 );

my $gh = new GeoHash();
#print $gh->add(9,16);
#TODO:$gh->encode(Point->new(1,2));
